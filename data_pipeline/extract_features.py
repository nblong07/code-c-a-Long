#!/usr/bin/env python3
"""
Feature extraction using Google SigLIP 2 Giant (ViT-gopt-16-SigLIP2-384, 1536d, FP16).
Hardware: AMD 7000 Series (13-14 CPU threads, 85% of 16 cores) + NVIDIA RTX 3050 6GB VRAM (88% cap = 5.28 GB).
"""
...
class KeyframeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        path = self.paths[idx]
        try:
            # OpenCV WebP decode (faster than PIL on Windows)
            img_bgr = cv2.imread(path, cv2.IMREAD_COLOR)
            if img_bgr is not None:
                img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
                img_resized = cv2.resize(img_rgb, (384, 384), interpolation=cv2.INTER_CUBIC)
                # normalize: pixel / 255.0 -> [0,1], then (x - 0.5) / 0.5 = x * 2 - 1
...
def parse_args():
    ...
    p.add_argument("--batch-size", type=int, default=24,
                   help="Batch size for inference (default: 24 fits RTX 3050 6GB VRAM at FP16).")
...
def extract_features_main():
    ...
    print("=" * 65)
    print("[INFO] Feature extraction: Google SigLIP 2 Giant (ViT-gopt-16-SigLIP2-384)")
    print(f"[INFO] Keyframes dir: {keyframes_dir}")
    print(f"[INFO] Model: {args.model} ({args.pretrained}) | 1536d FP16")
    print(f"[INFO] CPU threads: {OPTIMAL_CPU_THREADS} / {_cpu_cores}")
    print("=" * 65)

    if not os.path.exists(keyframes_dir):
        print(f"[ERROR] Directory not found: '{keyframes_dir}'")
        return False
    ...
    image_paths.sort()
    print(f"[INFO] Found {len(image_paths):,} keyframe images.")

    if len(image_paths) == 0:
        print("[ERROR] No valid images found.")
        return False

    # Skip if output files already cover all images
    if os.path.exists(args.output_features) and os.path.exists(args.output_paths):
        try:
            ...
            if len(existing_feats) == len(image_paths) and len(existing_paths) == len(image_paths):
                print(f"[INFO] [SKIP] All {len(existing_feats):,} vectors already extracted (dim {existing_feats.shape[1]}). Skipping.")
                return True
        except Exception:
            pass
    ...
    # Load model; cast to FP16 on CUDA for reduced VRAM and Tensor Core acceleration
    print(f"[INFO] Loading model: {args.model} ({args.pretrained})...")
    ...
    print(f"[INFO] Batch size={eff_batch_size} | DataLoader workers={num_loader_workers}")
    ...
    with torch.inference_mode():
        for batch_imgs, batch_paths, batch_valid in tqdm(dataloader, desc="Extracting SigLIP 2 Giant (FP16 CUDA)"):
    ...
    # Atomic write: save to .tmp then replace to avoid partial file on crash
    ...
    print(f"\n[OK] Extracted {len(features):,} vectors (dim={features.shape[1]}).")
    print(f"[OK] Features: '{args.output_features}' ({feat_size_mb:.1f} MB)")
    print(f"[OK] Paths:    '{args.output_paths}'")
    return True
