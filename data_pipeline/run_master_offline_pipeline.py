...
def main():
    ...
    parser.add_argument("--videos-dir", type=str, required=True, help="Đường dẫn thư mục chứa video mới (.mp4)")
    ...
    if not args.skip_keyframes:
        cmd1 = [python_exe, "data_pipeline/transnetv2_keyframes.py", "--input-folder", v_dir, "--output-base", kf_dir]
        if not run_step(cmd1, "Bước 1/5: Trích xuất Frame Thích ứng & Lọc mờ (TransNetV2 + Laplacian)"):
            return
    ...
    if not args.skip_asr:
        ...
        if not run_step(cmd2, "Bước 2/5: Trích xuất Lời thoại ASR (Faster-Whisper Large-v3-Turbo + Silero VAD)"):
            return
    ...
    cmd5 = [python_exe, "data_pipeline/extract_features.py", "--keyframes-dir", kf_dir, "--model", args.model]
    if not run_step(cmd5, "Bước 5/5: Trích xuất Vector Đặc trưng Thị giác (Google SigLIP 2 Giant 1152d)"):
        return
